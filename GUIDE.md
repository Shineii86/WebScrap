...
> **Everything you need to know to use WebScrap, step by step.**
...
**WebScrap** is a tool that captures the **full rendered HTML** from any website. It runs a real browser behind the scenes, so it works on JavaScript-heavy sites like React, Vue, Angular, and Next.js apps — not just static pages.
...
This is why WebScrap works on Next.js, React, Vue, and other frameworks that build their content with JavaScript.
...
1. **Click this link** (or the "Open in Colab" button in the README):
   ...
   👉 [Open WebScrap in Google Colab](https://colab.research.google.com/github/Shineii86/WebScrap/blob/main/notebook/WebScrap.ipynb)

2. **Sign in** with your Google account if prompted.

3. You'll see the notebook open in Google Colab. It looks like a document with boxes (cells) of code.

> 💡 **No GPU needed!** Unlike image generation tools, WebScrap runs entirely on CPU. You do NOT need to change the runtime type. The default "CPU" runtime works perfectly.
...
1. **Clones the WebScrap repo** — downloads the shared code modules
...
✅ Output directory: /content/webscrap_output
✅ Setup complete!
...
> - Or find the file at `/content/webscrap_export_*.zip` in Colab's file browser (folder icon on the left sidebar)
...
|------|-------|-------------|
| **Scraped HTML files** | `/content/webscrap_output/` | 📁 File browser → content → webscrap_output |
| **Metadata JSON** | `/content/webscrap_output/` | Same folder as HTML files |
| **Exported ZIP** | `/content/webscrap_export_*.zip` | 📁 File browser → content |
| **WebScrap repo** | `/content/WebScrap/` | 📁 File browser → content → WebScrap |
...
No! WebScrap runs entirely on CPU. Chromium doesn't need GPU acceleration for web scraping. The default Colab CPU runtime works perfectly.
</details>
...
[![GitHub](https://img.shields.io/badge/-GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Shineii86/WebScrap)
[![Telegram](https://img.shields.io/badge/-Telegram-2CA5E0?style=for-the-badge&logo=Telegram&logoColor=white)](https://telegram.me/Shineii86)
